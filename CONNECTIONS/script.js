import {players, getJersey, getName, getPosition, getTeam, getAge, getNumber} from "../players.js";

const grupConnect = [
    [["LETTERS", "SYMBOLS", "NUMBERS", "TWO LETTERS"], [players.baltimore[0], players.baltimore[2], players.baltimore[3], players.baltimore[4]], [players.baltimore[5], players.baltimore[7], players.baltimore[8], players.baltimore[9]], [players.baltimore[10], players.baltimore[11], players.baltimore[12], players.baltimore[14]], [players.baltimore[15], players.baltimore[16], players.baltimore[17], players.baltimore[18]]],
];

const boardState = {
    grup: grupConnect[Math.floor(Math.random() * grupConnect.length)],
    guess: [],
    lastAnswer: "",
    boxesSelected: 0,
    gameEnd: false,
...
function startConnections() {
    ...
    for(let i=1; i<boardState.grup.length; i++){
        boardState.grup[i].sort();
    }
...
function drawGrid(board) {
    if(boardState.grup.length == 5 && boardState.nrMistakes == 0){
        ...
        board.replaceChild(grid, board.children[3 - boardState.grup[0].length]);
...
function randomBoxes(grid){
    const allBoxes = [];
    for (let i = 1; i < boardState.grup.length; i++) {
        const currentGrup = boardState.grup[i]; 
        for (let j = 0; j < 4; j++) {
...
function drawBox(grid, row, col, player) {
      ...
      box.className = 'box';
      box.textContent = getName(player);

      box.style.backgroundImage = "url('" + getJersey(player) + "')";
      ...
      box.addEventListener("click", () =>{
        if(!boardState.gameEnd){
            ...
            if(computedStyle.backgroundColor == "rgb(128, 128, 128)"){
                ...
                const index = boardState.guess.indexOf(player);
                if(index > -1){
...
function drawBoxRight(board, answer){
    ...
    board.children[3 - boardState.grup[0].length].style.animationName = "";
    board.insertBefore(box, board.children[3 - boardState.grup[0].length]);
}
...
function checkGuess(){
    if(boardState.boxesSelected == 4 && boardState.gameEnd == false){
        ...
        for(let i=1; i<boardState.grup.length; i++){
            ...
            let found = true;
            for(let j=0; j<4; j++){
                if(guess[j] != currentGrup[j])
                    found = false;
            }
            if(found){
                solution = true;
                boardState.lastAnswer = boardState.grup[0][i-1];

                boardState.grup[0].splice(i-1, 1);
                boardState.grup.splice(i, 1);
...
function showNames(){
    for(let i=0; i<boardState.grup[0].length; i++){
        for(let j=0; j<4; j++){
